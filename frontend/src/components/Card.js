...
function Card({ card, handleCardClick, handleDeleteCardClick, handleDeleteCard, liking, disliking }) {
  ...
  const [isLiked, setIsLiked] = useState(likes && likes.some(i => i === currentUser._id));
  ...
  return (
    <div className='post'>
      ...
      {
        owner && owner._id === currentUser._id && <button ref={deleteButton} className='button post__delete'><img className='post__delete__image' alt='delete post' src={trashIcon} onClick={handleDelete} /></button>
      }
      <div className='post__container'>
        ...
        <div className='info'>
          ...
          <p className='like'>{likes && likes.length}</p>
        </div>
      </div>
    </div >
  );
